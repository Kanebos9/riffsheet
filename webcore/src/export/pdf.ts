...
async function withPrintRender<T>(
  ...
  try {
    ...
    // EVERY TAB TRACK'S OWN TUNING (P3), in printed order. The old live-only call fixed one
    // reordering bug but left imported tablature entirely uncaptioned. Each exact track lookup
    // is now independent, so a guitar above a bass prints six guitar letters above four bass
    // letters and neither can fall back to the other's bounds or tuning.
    const letters = built.score.tracks.flatMap((track, trackIndex) => {
      if (!track.staves.some((staff) => staff.showTablature)) return [];
      return stringLettersFromBounds(
        api.renderer.boundsLookup,
        tuningLowToHighFromScore(built.score, trackIndex),
        undefined,
        null,
        trackIndex
      );
